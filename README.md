...
- **프론트(웹·BFF):** **Next.js 15**, **React 19** — 서비스별 **`services/<svc>/web/`**. Usage·Billing·Identity·Notification은 App Router, **팀은 Pages Router**(`team-web`, `basePath=/teams`). **단일 오리진·경로 통합**은 **`web-edge`** Nginx(`/dashboard`, `/teams`, …). **`web-mfe/`·`/mfe/usage` Module Federation은 제거**됨.
- 상세: `docs/architecture.md` §10.2, §13.3 · 웹 경계: `docs/contracts/web-split-boundary.md` · 팀 BFF: `docs/contracts/web-team-bff.md`
...
- **web-edge 시크릿 주입(배포 준비):** `docker/web-edge/nginx.conf.template`는 `${GATEWAY_SHARED_SECRET}`를 포함하며, `docker-compose.yml`의 `web-edge`가 `envsubst`로 런타임 렌더링합니다. 운영 배포에서는 `.env` 고정값 대신 CI/CD 단계에서 **Secret Manager(Vault/AWS/GCP 등)** 값을 컨테이너 환경변수 `GATEWAY_SHARED_SECRET`로 주입하는 구조를 사용하세요.
...
- **Compose `profile: web`의 team-service**는 루트 **`docker-compose.yml`**에서 호스트에 노출하는 포트만 **`TEAM_SERVICE_HOST_PORT`**(기본 **8093**)로 매핑합니다. 호스트에서 JVM으로 team을 띄울 때 쓰는 **`TEAM_SERVICE_PORT`**(예: **8094**)와 이름을 분리해, 예전처럼 같은 변수가 Compose 호스트 바인딩과 겹치면서 Docker Desktop이 호스트 **8094**를 잡고 `bootRun`이 실패하는 경우를 피합니다. `TEAM_SERVICE_PORT`는 Compose `team-service` 컨테이너 안에서는 여전히 **8093**으로 고정이며, **team-web**가 같은 스택에서 부를 때는 기본으로 **`http://team-service:8093`** 을 씁니다(호스트 전용 team이면 `.env`의 **`WEB_TEAM_SERVICE_URL`**을 `host.docker.internal` + 실제 포트로 맞춤).
...
- GitHub Actions CI·로컬/CI Docker 빌드 캐시: `docs/CI.md` (“Docker 빌드 캐시 (로컬 vs CI)” 절)
