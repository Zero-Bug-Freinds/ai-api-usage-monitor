...
- **team-service / billing-service**는 각각 `TEAM_SERVICE_PORT`·`BILLING_SERVICE_PORT` 미설정 시 Spring 기본값이 둘 다 **8093**이라 동시 기동 시 충돌합니다. `scripts/bootrun.ps1`(·`bootrun.sh`)과 VS Code `bootRun: *`는 team **8094**·billing **8095**를 기본으로 넣어 분리합니다. billing 포트를 바꾼 경우 API Gateway의 **`GATEWAY_BILLING_URI`**를 동일 포트로 맞춥니다(`.env.example`). 지출 API(`GET /api/v1/expenditure/api-keys` 등) **500·502·DB 연결** 문제의 로컬 점검표는 **`docs/billing-service-overview-20260412.md` §6.4** 를 본다.
