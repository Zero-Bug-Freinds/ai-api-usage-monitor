...
switch ($Service) {
    ...
    'ai-agent-service' { if (-not $env:AI_AGENT_SERVICE_PORT) { $env:AI_AGENT_SERVICE_PORT = '8096' } }
    Default { }
}
...
try {
    ...
    if ($Service -eq 'ai-agent-service' -and -not (Test-Path -LiteralPath $gwBat) -and -not (Test-Path -LiteralPath $gwSh)) {
