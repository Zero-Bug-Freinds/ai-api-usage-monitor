...
%{if var.enable_ec2_rabbitmq}
    install_host_rabbitmq() {
      export RABBITMQ_USER='${var.ec2_rabbitmq_user}'
      export RABBITMQ_PASSWORD='${local.ec2_rabbitmq_password_escaped}'
      export DEPLOY_STATE_DIR="/var/lib/ai-api-usage-monitor-deploy"
      if [ -x /opt/${var.project_name}/scripts/deploy/install-ec2-rabbitmq.sh ]; then
        bash /opt/${var.project_name}/scripts/deploy/install-ec2-rabbitmq.sh
      else
        mkdir -p "$DEPLOY_STATE_DIR"
        chmod 700 "$DEPLOY_STATE_DIR"
        docker rm -f aio-host-rabbitmq 2>/dev/null || true
        docker run -d --name aio-host-rabbitmq --restart unless-stopped \
          -p 127.0.0.1:5672:5672 -p 127.0.0.1:15672:15672 \
          -e "RABBITMQ_DEFAULT_USER=$RABBITMQ_USER" \
          -e "RABBITMQ_DEFAULT_PASS=$RABBITMQ_PASSWORD" \
          rabbitmq:3-management-alpine
        cat >"$DEPLOY_STATE_DIR/terraform-rabbitmq.env" <<MQENV
# Generated by Terraform user-data (inline fallback)
...
MQENV
        chmod 600 "$DEPLOY_STATE_DIR/terraform-rabbitmq.env"
      fi
    }
    install_host_rabbitmq
%{endif}
