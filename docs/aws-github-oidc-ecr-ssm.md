...
- **Agent LLM keys (`AI_AGENT_*`)**: prefer **Secrets Manager** secret `prod/ai-agent/credentials` (override with env `AGENT_CREDENTIALS_SECRET_ID`). [`scripts/deploy/on-instance-compose-roll.sh`](../scripts/deploy/on-instance-compose-roll.sh) merges JSON keys prefixed with `AI_AGENT_` into `compose.env` on each roll (overrides `.env.deploy`). EC2 **instance profile** needs `secretsmanager:GetSecretValue` on that secret ARN — Terraform grants this via `ec2_agent_credentials_secret_ids` on [`infra/terraform/modules/compute_stack`](../infra/terraform/modules/compute_stack/main.tf) (`aws_iam_role_policy.ec2_agent_credentials_secrets`). After `terraform apply`, re-run compose roll; roll logs must show `Merged agent credentials from Secrets Manager`, not `GetSecretValue failed`. Set `AGENT_CREDENTIALS_FROM_SECRETS_MANAGER=false` to disable.
...
2. **Per Environment variables:** `AWS_REGION`, optional `ECR_REPOSITORY_PREFIX`, `ALB_TARGET_GROUP_ARN` (set for post-Release auto roll), `SSM_DEPLOY_ROOT` (use `terraform output ssm_deploy_root_default`), optional `TARGET_PORT` (use `terraform output alb_target_port`, or leave unset — workflows default to **8888**), **`NEXT_PUBLIC_WEB_EDGE_ORIGIN`** (required when any `*-web` image builds) for **`release.yml` web image build-args** — see §10.1 below. **`NEXT_PUBLIC_IDENTITY_WEB_ORIGIN`** and **`NEXT_PUBLIC_USAGE_WEB_ORIGIN`** are optional: Release copies **`WEB_EDGE`** when they are unset. **Release / Deploy OIDC role ARNs** are pinned in [`.github/workflows/release.yml`](../.github/workflows/release.yml) and [`.github/workflows/deploy.yml`](../.github/workflows/deploy.yml) `env`; after `terraform apply`, ensure those ARNs match **`terraform output aws_release_role_arn`** / **`aws_deploy_role_arn`**, or switch the workflows back to `vars.AWS_*_ROLE_ARN` if you prefer Environment-stored ARNs.

### 10.1 `NEXT_PUBLIC_*` · ALB DNS (web images)

| Fact | Detail |
|------|--------|
| When set | **Docker build** in `release.yml` (`Push identity-web`, `usage-web`, `billing-web`, `team-web`, `notification-web`, `agent-web`) — values from Environment **`vars`**, not from EC2 `.env.deploy`. |
| **Required** | **`NEXT_PUBLIC_WEB_EDGE_ORIGIN`** — public browser URL via ALB, **no trailing slash**. Example: `http://ai-api-usage-monitor-alb-787442144.ap-northeast-2.elb.amazonaws.com` |
| **Optional (recommended same URL)** | **`NEXT_PUBLIC_IDENTITY_WEB_ORIGIN`** — if unset, Release uses **`NEXT_PUBLIC_WEB_EDGE_ORIGIN`** so Identity/login links are not baked as empty strings. Set both to the **same ALB URL** for clarity. |
| **Optional** | **`NEXT_PUBLIC_USAGE_WEB_ORIGIN`** — if unset, Release uses **`WEB_EDGE`** (`identity-web` only). |
| After compose roll only | Backend/env on EC2 updates; **sidebar cross-app links stay on the old origin** baked into existing `*-web` images until those images are **rebuilt and redeployed**. |
| ALB hostname | `terraform output alb_dns_name` → `http://<name>-<id>.ap-northeast-2.elb.amazonaws.com` (no trailing slash). **Recreating the ALB changes `<id>`**; update GitHub `NEXT_PUBLIC_WEB_EDGE_ORIGIN` (and optional duplicates) and run Release with **`web_shared`** or **`force_rebuild_all`**. |
| `.env.deploy` | Documents deploy secrets; `NEXT_PUBLIC_*` there are **comments/samples** for operators — they do **not** override Next bundles at runtime (`usage-web` in `docker-compose-prod.yml` does not inject them). |
| Runtime mitigation | `@ai-usage/shell` uses current browser origin on `.elb.amazonaws.com` when it disagrees with the baked origin ([`packages/shell/README.md`](../packages/shell/README.md)). Prefer correct GitHub vars + rebuild anyway. |

**Checklist after ALB replace:** `terraform output alb_dns_name` → update **staging/production** `NEXT_PUBLIC_WEB_EDGE_ORIGIN` (and optionally mirror the same value into `NEXT_PUBLIC_IDENTITY_WEB_ORIGIN`) → Release (**force_rebuild_all** if path filters skipped web) → `on-instance-compose-roll.sh` with new `IMAGE_TAG` → hard-refresh browser; verify sidebar 「알림」 `<a href>` host matches the ALB you opened.
