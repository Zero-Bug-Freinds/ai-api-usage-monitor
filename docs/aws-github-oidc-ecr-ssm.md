...
2. **Per Environment variables:** `AWS_REGION`, optional `ECR_REPOSITORY_PREFIX`, `ALB_TARGET_GROUP_ARN` (set for post-Release auto roll), `SSM_DEPLOY_ROOT` (use `terraform output ssm_deploy_root_default`), optional `TARGET_PORT` (use `terraform output alb_target_port`, or leave unset — workflows default to **8888**), **`NEXT_PUBLIC_WEB_EDGE_ORIGIN`** and **`NEXT_PUBLIC_IDENTITY_WEB_ORIGIN`** (and optional `NEXT_PUBLIC_MFE_USAGE_REMOTE_URL`, `NEXT_PUBLIC_USAGE_WEB_ORIGIN`) for **`release.yml` web image build-args** — see §10.1 below. **Release / Deploy OIDC role ARNs** are pinned in [`.github/workflows/release.yml`](../.github/workflows/release.yml) and [`.github/workflows/deploy.yml`](../.github/workflows/deploy.yml) `env`; after `terraform apply`, ensure those ARNs match **`terraform output aws_release_role_arn`** / **`aws_deploy_role_arn`**, or switch the workflows back to `vars.AWS_*_ROLE_ARN` if you prefer Environment-stored ARNs.
...
| ALB hostname | `terraform output alb_dns_name` → `http://<name>-<id>.ap-northeast-2.elb.amazonaws.com` (no trailing slash). **Recreating the ALB changes `<id>`**; update GitHub `NEXT_PUBLIC_*` and run Release with **`web_shared`** or **`force_rebuild_all`**. |
...
**Checklist after ALB replace:** `terraform output alb_dns_name` → update **staging/production** `NEXT_PUBLIC_WEB_EDGE_ORIGIN` / `NEXT_PUBLIC_IDENTITY_WEB_ORIGIN` → Release (**force_rebuild_all** if path filters skipped web) → `on-instance-compose-roll.sh` with new `IMAGE_TAG` → hard-refresh browser; verify sidebar 「알림」 `<a href>` host matches the ALB you opened.
