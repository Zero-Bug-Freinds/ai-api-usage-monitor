...
세 소비자가 RabbitMQ에 연결되어 있고 purge·ACK가 정상 처리되면 Identity 최종 삭제까지 진행된다. **남은 갭:** 설정 UI(§7.7), pending stuck 운영(§7.1 #7), notification·agent 병행 정리(§7.5·§7.6).
...
| 2 | **팀 맥락 usage 삭제·익명화** | 옵션 B·C (§7.4) — 현재는 A만 적용 |
...
2. ~~**usage-service** — listener + 개인 purge + ACK~~ ✅
