...
엣지 뒤에서는 브라우저가 **`http://localhost:8888`** 등 한 오리진으로 붙고, **`/api/v1`** 는 **`/api/v1/`** 로 **308** 리다이렉트된 뒤 **`/api/v1/*`** 가 게이트웨이로 프록시된다(로컬 Nginx는 스트리밍·장시간 응답을 위해 **`proxy_buffering off`**·긴 timeout — `docker/web-edge/nginx.conf`). 팀이 변경 시 본 문서·`docker/web-edge/nginx.conf`·`docker-compose.yml`·`application.yml`을 함께 갱신한다.
