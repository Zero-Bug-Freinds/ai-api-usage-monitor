...
엣지 뒤에서는 브라우저가 **`http://localhost:8888`** 등 한 오리진으로 붙고, **`/api/v1/*`** 는 게이트웨이로 프록시된다(`docker/web-edge/nginx.conf`). 팀이 변경 시 본 문서·`docker-compose.yml`·`application.yml`을 함께 갱신한다.
