...
- 로컬에서 Identity와 게이트웨이 **포트가 겹치면** `IDENTITY_SERVICE_URL`과 `API_GATEWAY_URL`을 팀이 분리해 맞춘다([`services/usage-service/web/.env.example`](../../services/usage-service/web/.env.example) 및 루트 `.env.example` 주석).
