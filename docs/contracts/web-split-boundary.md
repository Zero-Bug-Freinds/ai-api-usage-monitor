...
### 2.3 Team `web/` (팀 관리·Team BFF)

Next `basePath`는 **`/teams`**. 브라우저 접두는 아래와 같다.

| 경로 접두 | 설명 |
|-----------|------|
| `/teams` | 팀 관리 홈(UI) |
| `/teams/api/team/v1/*` | Team BFF → Team Service `/api/v1/*` |
| `/teams/api/auth/session` | 세션 확인 BFF → Identity `/api/auth/session` |

### 2.4 로컬 Compose `web-edge`(Nginx 정본)
...
| 앱 | `config.matcher` |
|----|------------------|
| Identity `web/` | `/settings/:path*`, `/organizations/:path*` |
| Usage `web/` | 정적·API 제외 전역 매처(BFF `/dashboard/api/**`는 코드에서 통과) |
| Team `web/` | 팀 앱 내부 규칙(현재 `/teams` 진입은 Team 앱 소유) |
...
- Team `/teams/api/team/v1/*`·팀 생성/초대: [web-team-bff.md](./web-team-bff.md)
