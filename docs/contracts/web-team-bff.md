...
#### 7.3.1 사이드바·Usage 대시보드 링크(2026-05)

- `team-management-view.tsx` 는 **`/teams` 한 페이지**에서 팀 목록·CRUD(아코디언)를 처리한다. **Module Federation·`/mfe/usage` 없음.**
- `@ai-usage/shell` 팀 서브메뉴(팀 목록이 사이드바에 있을 때): **「대시보드」→ `/dashboard`**, **「멤버 상세」→ `/dashboard/team?viewTeamId=…&tab=member`** (Usage `web`, web-edge 경유).
- **「팀」메인 메뉴** → **`/teams`** (`resolveTeamShellEntryHref`, 배포 origin은 Release `NEXT_PUBLIC_WEB_EDGE_ORIGIN` + 브라우저 현재 호스트 보정).
