...
**의미**: 특정 달(`monthStart`)에 대해, **해당 사용 이벤트의 API 키(`apiKeyId`)·프로바이더**의 **KST 달력 월**에 대해 `daily_expenditure_agg`를 합산한 누적 지출(지출 API의 키·프로바이더·해당 월 전체 기간과 동일 스코프)이 Identity에 설정된 **그 키·프로바이더 월 예산** 대비 **10%, 20%, …, 100%** 구간을 **각각 처음 넘는 순간**마다 알림용으로 발행됩니다. **키별 월 예산이 Identity에 없거나 0 이하**이면 임계 이벤트는 발행되지 않습니다(`BILLING_IDENTITY_*` 비활성 시 예산 조회가 비어 동일하게 스킵될 수 있음).
