...
- **예산 출처**: 금액은 billing이 Identity에서 **키·프로바이더별**로 조회한 예산 스냅샷 기준이며, 유저 전체 예산이 아닙니다. Identity JSON의 `provider` 값(`GEMINI` 등)과 usage 이벤트의 `AiProvider`(`GOOGLE` 등) **문자열 매핑**은 `IdentityBudgetClient`가 수행합니다(`docs/billing-identity-budget.md`). 상세 출처는 `GET /api/v1/expenditure/monthly-budget-authority`(billing) 등과 별개입니다.
