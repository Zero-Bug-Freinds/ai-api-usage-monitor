...
| 월 예산 임계 도달(개인·Identity 키·프로바이더 스코프; `UsageRecordedEvent.apiKeySource=team` 제외) | `billing.events` | `billing.budget.threshold.reached` | `BILLING_RABBIT_BUDGET_OUT_ENABLED=false` |
| 월 예산 임계 도달(팀 API 키·Team DB에 등록된 키만) | `billing.events` | `billing.team.budget.threshold.reached` | `BILLING_RABBIT_TEAM_BUDGET_OUT_ENABLED=false` |
...
- 예산 임계(개인): `BILLING_RABBIT_BUDGET_OUT_EXCHANGE`, `BILLING_RABBIT_BUDGET_OUT_ROUTING_KEY`
- 예산 임계(팀 API 키): `BILLING_RABBIT_TEAM_BUDGET_OUT_EXCHANGE`, `BILLING_RABBIT_TEAM_BUDGET_OUT_ROUTING_KEY`
...
## 2) `BillingBudgetThresholdReachedEvent` (개인·Identity API 키 — 월 예산 임계 구간 통과)

**의미**: 특정 달(`monthStart`)에 대해, **해당 사용 이벤트의 API 키(`apiKeyId`)·프로바이더**의 **KST 달력 월**에 대해 `daily_expenditure_agg`를 합산한 누적 지출(지출 API의 키·프로바이더·해당 월 전체 기간과 동일 스코프)이 Identity에 설정된 **그 키·프로바이더 월 예산** 대비 **임계 구간**을 **각각 처음 넘는 순간**마다 알림용으로 발행됩니다. **전제:** `UsageRecordedEvent`의 `apiKeySource`가 **`team`인 호출은 개인 `daily_expenditure_agg`·`monthly_expenditure_agg`를 갱신하지 않으므로** 이 이벤트 경로로는 평가되지 않습니다(팀 키는 §2.1). **키별 월 예산이 Identity에 없거나 0 이하**이면 임계 이벤트는 발행되지 않습니다(`BILLING_IDENTITY_*` 비활성 시 예산 조회가 비어 동일하게 스킵될 수 있음).

- 현재 구현(테스트 목적): **1%, 2%, …, 100%** (1% steps)
- 추후 운영 기본값(의도): **10%, 20%, …, 100%** (10% steps)
  - 전환은 `services/billing-service/.../BudgetThresholdEventPublisher.java`의 `PERCENT_STEP` 값으로 제어한다.
...
| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
| `schemaVersion` | number | 예 | `2` |
| `occurredAt` | string (ISO-8601 instant) | 예 | 발행 시각 |
| `monthStart` | string (ISO date) | 예 | 해당 월의 1일 |
| `thresholdPct` | number | 예 | 예: `0.01` … `1` (예산 대비 비율 임계값, 현재 1% 단위; 운영은 10% 단위로 전환 가능) |
| `monthlyTotalUsd` | number | 예 | 임계 통과 후 **해당 키·프로바이더**의 KST 달력 월 합계 USD(`daily_expenditure_agg` 합산; `monthly_expenditure_agg` 전 프로바이더 합과는 다를 수 있음) |
| `monthlyBudgetUsd` | number | 예 | Identity에서 조회한 **해당 키·프로바이더** 월 예산 USD(발행 시점 스냅샷). 키 예산이 없거나 0 이하면 메시지 자체가 나가지 않음 |
| `apiKeyAlias` | string | 아니오 | Identity에서 조회한 API 키 별칭(표시용). 없을 수 있음 |
...
- **중복·재시도**: 동일 임계를 여러 번 넘지 않는 한 발행 빈도는 제한적이나, **알림 멱등 키**(예: `apiKeyId` + `monthStart` + `thresholdPct`, 헤더 `subjectType`이 `API_KEY`인 경우)를 두는 것을 권장합니다.
  - notification-service 인앱 구현은 `thresholdPct`를 퍼센트 정수로 정규화해(`pct{n}`) dedupe 키로 사용한다.
- **예산 출처**: 금액은 billing이 Identity에서 **키·프로바이더별**로 조회한 예산 스냅샷 기준이며, 유저 전체 예산이 아닙니다. Identity JSON의 `provider`는 **`GOOGLE`** 이 canonical이며, 레거시 **`GEMINI`** 행은 billing `AiProvider.GOOGLE` 요청과 호환 매칭됩니다(`IdentityBudgetClient`, `docs/billing-identity-budget.md`). 상세 출처는 `GET /api/v1/expenditure/monthly-budget-authority`(billing) 등과 별개입니다.

---

## 2.1) `BillingTeamApiKeyBudgetThresholdReachedEvent` (팀 API 키 월 예산 임계 구간 통과)

**의미**: `UsageRecordedEvent`가 **팀 소스**(`apiKeySource`가 `team` 등)이고 `teamId`·`teamApiKeyId`가 유효할 때, billing이 **팀 API 키별 월 집계**(팀 키 전용 aggregation)와 Team DB에 저장된 **해당 팀 API 키의 `monthlyBudgetUsd`** 를 기준으로, **KST 달력 월** 누적 지출이 임계 구간을 **각각 처음 넘는 순간**마다 알림용으로 발행합니다. 개인 키용 `BillingBudgetThresholdReachedEvent`(§2)와 **exchange는 같아도 routing key가 다릅니다.** 팀 키 예산이 없거나 0 이하면 발행하지 않습니다.

- 임계 스텝: **구현은 `TeamApiKeyBudgetThresholdEventPublisher`의 `PERCENT_STEP`(현재 1% 단위)** 과 동일한 방식으로 생성된 목록을 사용합니다(§2의 개인 키 경로와 코드 구조가 대칭적입니다).
- **등록된 팀 키만**: 집계·예산 행이 billing read model에 없으면 처리되지 않습니다(`BillingRecordedService.maybeProcessTeamKeyBudgetThreshold`).

| 구분 | 내용 |
|------|------|
| Java 타입 | `services/billing-service`: `com.eevee.billingservice.events.BillingTeamApiKeyBudgetThresholdReachedEvent` |
| `schemaVersion` (현재) | `1` |
| 발행 시점 | 사용 이벤트 처리 트랜잭션 **커밋 이후** (`TeamApiKeyBudgetThresholdEventPublisher`) |
...
| `schemaVersion` | number | 예 | `1` |
| `occurredAt` | string (ISO-8601 instant) | 예 | 발행 시각 |
| `teamId` | number | 예 | 팀 ID |
| `triggerUserId` | string | 예 | 해당 usage를 유발한 사용자(빈 문자열일 수 있음; 소비자는 방어적으로 처리) |
| `teamApiKeyId` | number | 예 | Team이 발행한 팀 API 키 엔티티 ID |
| `provider` | string | 예 | 표시·필터용 공급자 코드 |
| `apiKeyAlias` | string | 예 | 팀 키 별칭(없으면 빈 문자열 가능) |
| `monthStart` | string (ISO date) | 예 | 해당 월 1일 |
| `thresholdPct` | number | 예 | 예산 대비 비율 임계값(§2와 동일한 0~1 스케일) |
| `monthlyTotalUsd` | number | 예 | 임계 통과 **후** 해당 팀 API 키의 KST 달력 월 합계 USD |
| `monthlyBudgetUsd` | number | 예 | Team DB 기준 해당 팀 API 키 월 예산 USD |

### AMQP 메시지 헤더

| 헤더 | 설명 |
|------|------|
| `subjectType` | `TEAM_API_KEY` |
| `teamId`, `teamApiKeyId` | 문자열 형태의 숫자 ID |
| `triggerUserId` | 있으면 설정 |
| `provider`, `apiKeyAlias` | 있으면 설정 |

### 소비자 구현 체크리스트

- **전용 큐 권장**: `billing.budget.threshold.reached`와 **다른 큐**로 바인딩해 JSON 스키마를 분리합니다(기본 구현: notification-service의 `notification.billing.team.events` 등).
- **멱등**: 팀원 **각 사용자**마다 한 번씩 인앱이 필요하면 `(teamId, teamApiKeyId, targetUserId, monthStart, thresholdPct)` 조합으로 dedupe 키를 설계합니다. notification-service는 `NotificationDelivery.dedupeKey`로 이를 보장합니다.
- **수신자 결정**: billing 본문만으로는 팀 전체 멤버 목록이 없으므로, downstream이 **team-service HTTP** 등으로 멤버를 조회하는 패턴이 맞습니다(구현: `services/notification-service`의 `TeamServiceClient`).
...
| 예산 임계 발행(개인·Identity 키) | `.../service/BudgetThresholdEventPublisher.java` |
| 예산 임계 발행(팀 API 키) | `.../service/TeamApiKeyBudgetThresholdEventPublisher.java` |
