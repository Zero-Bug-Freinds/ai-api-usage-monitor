...
- **Next.js 15**·**React 19** 모노레포에서 **App Router** 기반 `web/`(BFF·운영 UI)과 **Module Federation remote** 요구를 동시에 만족시키기 위해, **Usage·Team** 에 **`web-mfe/`**(Pages Router)를 둔다.
- **호스트**는 주로 **`apps/web`**(`web-host`)가 `remotes`로 `usage`·`team` remoteEntry를 로드한다.
- **브라우저 단일 오리진**으로 여러 도메인 UI를 붙이는 경로 통합은 Nginx **`web-edge`** 가 담당한다(일부 환경에서는 `identity-service/web`의 `rewrites`를 병행할 수 있음). MF 전용 포트는 개발 시 호스트·remote가 직접 맞춘다.
