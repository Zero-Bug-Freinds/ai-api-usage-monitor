...
Identity **`web/`** 에는 MF 플러그인을 두지 않고, **rewrite 게이트웨이** 역할만 유지하는 것을 권장한다.
...
## 4. Identity `rewrites`와의 관계

- **페이지·BFF 경로**(`/dashboard`, `/billing`, `/notifications`, `/api/team/v1/...` 등)를 서비스별 `web`으로 넘기는 것은 **Identity `next.config.ts`** 의 **`rewrites`** 정본이다. 새 라우트를 “같은 오리진에서 다른 서비스로” 노출할 때는 **반드시 여기에 추가**하고 [`web-split-boundary.md`](contracts/web-split-boundary.md)를 갱신한다.
- **MF remote**(`/_next/static/chunks/remoteEntry.js` 등)는 보통 **호스트↔remote 직접 통신**이며, 필요 시 CORS·오리진은 배포 환경에서 맞춘다.
...
2. 새 브라우저 공개 경로가 다른 서비스 `web`으로 가야 하면 **Identity `rewrites`** + **`web-edge`**(사용 시) 동시 수정.
