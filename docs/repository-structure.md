...
- **단일 도메인:** 엣지 역프록시로 경로를 합친다. 로컬 Compose는 **`web-edge`** + `docker/web-edge/nginx.conf`(예: `/dashboard*` → usage `web`, `/api/v1/*` → API Gateway, 나머지 → identity `web`). 상세: `docs/architecture.md` §10.2.
