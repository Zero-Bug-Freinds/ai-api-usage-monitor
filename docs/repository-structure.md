...
- **`docs/`**: 팀 문서의 정본.
- **`docker-compose.yml`**: **저장소 루트**에 두어 로컬에서 DB·브로커·캐시 등을 한 번에 기동하기 쉽게 한다. **배포·로컬 스택은 `docs/architecture.md` §10.1 패턴 B(백엔드·프론트 이미지 분리·Compose)** 를 따르며, 애플리케이션은 **서비스·`apps/web`별 이미지**로 올리고 Compose로 연결한다.
...
- **컨테이너 배포(패턴 B)**: `apps/web/Dockerfile`(Next.js `output: 'standalone'`)로 **프론트 전용 이미지**를 빌드한다. 로컬에서 Compose로 올릴 때는 루트 `docker-compose.yml`의 **`profile: web`** 서비스를 참고한다.
- **인증 쿠키·BFF 엔드포인트·401 처리** 계약: [`docs/contracts/web-identity-bff.md`](contracts/web-identity-bff.md).
- **Usage BFF** (`/api/usage/**` → 게이트웨이 `/api/v1/usage/**` 호출 맵·환경 변수): [`docs/contracts/web-gateway-bff.md`](contracts/web-gateway-bff.md).
