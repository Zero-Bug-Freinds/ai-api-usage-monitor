...
- 책임 범위
  ...
  - **콘솔 사이드바 미확인 배지:** `@ai-usage/shell`이 `unread-count`를 주기적으로 조회해 알림 메뉴에 숫자를 표시한다. Notification `web`에서 읽음·모두 읽음·팀 초대 처리가 성공하면 `dispatchNotificationsChanged()`로 **`ai-usage:notifications-changed`** 를 발행해 배지를 즉시 재조회한다(폴링만으로는 지연될 수 있음). 계약: [`docs/contracts/web-notification-bff.md`](contracts/web-notification-bff.md) §4.7, [`packages/shell/README.md`](../packages/shell/README.md).
