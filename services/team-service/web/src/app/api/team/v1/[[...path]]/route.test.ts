...
afterEach(() => {
  vi.restoreAllMocks()
  delete process.env.TEAM_SERVICE_URL
})
...
describe("team-web BFF route", () => {
  it("returns 401 when cookie is missing", async () => {
    process.env.TEAM_SERVICE_URL = "http://localhost:8093"
  ...
  it("proxies GET to team-service", async () => {
    ...
    const fetchMock = vi.fn(async (url: string, init?: RequestInit) => {
      expect(url).toBe("http://localhost:8093/api/v1/me/teams")
  ...
  it("proxies POST body to team-service", async () => {
    process.env.TEAM_SERVICE_URL = "http://localhost:8093"
    const fetchMock = vi.fn(async (url: string, init?: RequestInit) => {
      expect(url).toBe("http://localhost:8093/api/v1/teams")
