...
- Identity 연동이 **비활성화** 되어 있거나, `base-url`이 비어 있거나, HTTP **호출 실패**·**404** 시 API의 `monthlyBudgetUsd`는 **`null`** 입니다.
- 연동은 성공했지만 키에 예산이 없으면 **`0`** 을 반환합니다.
- 예산이 없더라도 Billing 지출 집계(`totalCostUsd`)는 정상 동작합니다. UI는 `null`과 `0`을 구분해 표시합니다(`docs/billing-identity-budget.md` §Billing 지출 UI).

### 설정 키 (`application.yml` / 환경변수)

[`IdentityProperties`](src/main/java/com/eevee/billingservice/config/IdentityProperties.java)는 **`billing.identity.*`** 만 읽습니다. YAML은 **`billing:`** 블록 아래에 둡니다.

| Spring 키 | 환경변수 | 설명 |
|-----------|----------|------|
| `billing.identity.enabled` | `BILLING_IDENTITY_ENABLED` | `true`일 때만 HTTP 조회. `application.yml` 기본 `false`; **Docker Compose 로컬** 기본 `true` (`.env.example`과 동일). |
| `billing.identity.base-url` | `BILLING_IDENTITY_BASE_URL` | Identity base URL. 비어 있으면 호출 스킵. |
| `billing.identity.budget-path-template` | `BILLING_IDENTITY_BUDGET_PATH` | path 템플릿. env 미설정 시 기본 `/api/identity/v1/users/budget?email={userId}`. `{userId}`는 `X-User-Id`로 치환. |
...
Compose는 루트 `.env.example`의 `http://identity-service:8080` 값을 그대로 쓰면 됩니다.

바인딩 회귀 테스트: [`IdentityPropertiesBindingTest`](src/test/java/com/eevee/billingservice/config/IdentityPropertiesBindingTest.java).
...
상세: [`docs/billing-identity-budget.md`](../../docs/billing-identity-budget.md) §트러블슈팅.

- **YAML 경로**
  - `billing.identity.*`는 **`billing:` 블록** 아래여야 한다. top-level `identity.identity.*` 등은 바인딩되지 않아 연동이 꺼진 것처럼 `monthlyBudgetUsd: null`이 된다.

- **인증 헤더 미전송**
  - 현재 구현(`IdentityBudgetClient`)은 예산 조회 요청에 별도 인증 헤더/토큰을 붙이지 않습니다.
  - Identity가 인증을 요구하는 엔드포인트라면, 이 연동은 예산을 **`null`** 로 처리하게 됩니다.

- **조용히 실패할 수 있음**
  - 404는 **`null`** 로 취급한다(활성 키 없음 등). 연동 성공·예산 0은 **`0`**.
  - 그 외 상태코드/예외도 예산을 **`null`** 로 처리하며, 로그는 `debug` 레벨로만 남습니다.
  - 예산이 계속 `null`이면 `BILLING_IDENTITY_*` 주입·`base-url` reachability·Identity 예산 API 응답을 확인한다.
