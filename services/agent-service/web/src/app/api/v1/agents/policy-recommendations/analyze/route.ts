...
export async function POST(request: NextRequest) {
  ...
  if (origin == null) {
    return NextResponse.json(
      {
        message: "recommendation analyze backend 호출에 실패했습니다. (agent-service 연결 불가)",
      },
      { status: 502 },
    )
  }
  ...
  try {
    const response = await fetchWithTimeout(`${origin}/api/v1/agents/policy-recommendations/analyze`, REQUEST_TIMEOUT_MS, {
      method: "POST",
    ...
    return NextResponse.json(
      {
        message: "recommendation analyze backend 호출에 실패했습니다.",
      },
      { status: 502 },
    )
  }
