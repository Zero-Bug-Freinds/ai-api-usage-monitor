...
export async function GET(
  ...
  if (origin == null) {
    return NextResponse.json(
      {
        message: "recommendation query backend 호출에 실패했습니다. (agent-service 연결 불가)",
      },
      { status: 502 },
    )
  }
  ...
  const targetUrl = `${origin}/api/v1/agents/policy-recommendations/${encodeURIComponent(keyId)}?${query}`
  ...
  try {
    ...
    return NextResponse.json(
      {
        message: "recommendation query backend 호출에 실패했습니다.",
      },
      { status: 502 },
    )
  }
